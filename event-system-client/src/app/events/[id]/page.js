...
export default async function EventPage({ params }) {
  ...
  // Отримуємо сам об'єкт (або url, залежно від структури відповіді)
  const coverUrl = event.cover?.url;

  let imageUrl = null;

  if (coverUrl) {
    // Якщо посилання вже повне (починається з http або https), не додаємо API_URL
    if (coverUrl.startsWith('http')) {
      imageUrl = coverUrl;
    } else {
      // Якщо посилання відносне (/uploads/photo.jpg), додаємо хост
      imageUrl = `${API_URL}${coverUrl}`;
    }
  }
